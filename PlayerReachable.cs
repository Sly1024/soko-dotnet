...
public class PlayerReachable
{
    ...
    // if this returns false, we need the participating boxes to be marked!
    ...
    public bool isBoxPullDeadLocked(int boxPos)
    {
        ...
        // we have a list of non-movable boxes, need to check if they're all on initial box positions
        var blockedBoxes = markedPositions.list;
        for (int i = markedPositions.count - 1; i >= 0; --i) {
            // level.table[] never changes, it stores the initial box positions
    ...
    private bool isBoxPullable(int boxPos)
    {
        ...
        bool isPullable(int box, int dirOff)
        {
            ...
            // T1 != BOX means it's empty, because we already checked if T1 == WALL.
