...
public class Level
{
    ...
    public Level(int[] table, int width)
    {
        ...
        DetectDeadCells();  // needs HeuristicDistances.Pushes/Pulls
        CountDeadCells();
    ...
    private void PreProcessTable()
    {
        ...
        if (boxes.Count != goals.Count) throw new ArgumentException($"Number of Boxes ({boxes.Count}) and Goals ({goals.Count}) don't match.");
    ...
    private static ulong Get64BitZobristString(Random rand)
    {
